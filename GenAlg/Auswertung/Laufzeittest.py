...
def start(proj_name     = None, 
    ...
    # for GA
        ...
        BS      = None,
        anhang      = None,
        show        = None):#Dokumentation des Verlaufs?
    ...
    if sim_config is None:
        ...
        else: print "proj_name not known: Be sure, that you choosed the right Simulation Configuration!"
    if stimulation is None:
        if proj_name is "Pyr_RS" or proj_name is "Pyr_IB": stimulation = "Input_0"
        else: print "proj_name not known: Be sure, that you choosed the right Stimulation!"
    if cell is None:
        if proj_name is "Pyr_RS" or proj_name is "Pyr_IB": cell = "L5TuftedPyrRS"
        else: print "proj_name not known: Be sure, that you choosed the right Cell!"
    if duration is None: duration = 500
    if dt is None: dt = 0.025
    if currents is None: currents = [3,0.2,0.3]
    if custom is None: custom = 1
    if anhang is None: anhang = " "
    
    ##############################
    
    if pop_size is None: pop_size = 50
    if max_generations is None: max_generations = 60
    if mode is None:
        if proj_name is "Pyr_RS": mode = 1
        elif proj_name is "Pyr_IB": mode = 3
        else: print "proj_name not known: Be sure, that you choosed the right mode!"
    if crossover_rate is None: crossover_rate = 0.25
    if thrFourier is None: thrFourier = 4
    if mutation_strength is None: mutation_strength = 0.05
    if mutation_rate is None: mutation_rate = 1/12.0
    if num_selected is None: num_selected = 30
    if num_co_points is None: num_co_points = 2
    if num_elites is None: num_elites = 1
    if tournament_size is None: tournament_size = 5
    ...
    config.close()  
    if BS == 1:
        channel = open("./GenAlg/Programm/Speicher/channel.txt","w") #Dateien leere, da später die Werte angehängt werden.
        location = open("./GenAlg/Programm/Speicher/location.txt","w")
        density = open("./GenAlg/Programm/Speicher/density.txt","w")
    else:
        channel = open("C:\Python27\GenAlg\Programm\Analyse\channel.txt","w") #Dateien leere, da später die Werte angehängt werden.
        location = open("C:\Python27\GenAlg\Programm\Analyse\location.txt","w")
        density = open("C:\Python27\BGenAlg\Programm\Analyse\density.txt","w")
...
def generate_conductance(random, args):
    ...
    chromosome = []
### klassenspezifische Kanäle:
    if args.get('modus')==2 or args.get('modus')==1:

        if args.get('modus')==1:
            chromosome = [10**int(-9), #ar
                       10**int(-10), #cal
                       10**int(-10), #cat
                       10**int(-9), #k2
                       10**int(-7), #ka
        ...
        else: #Anpassen, dass kahp, km nahezu konstant bleiben, da nur geringe adaption gewünscht
        ...
        if args.get('modus')==4: #chattering
...
def evaluate_NB(args):
    ...
    ausgabe = analyze_Neurons.analyze_Nonburst()
    if ausgabe['P'] == 0:
        return {'P':0}
    ...
    ### Normierung: Maximum: x = 0, Abweichungen im Rahmen von min/max: -100 <= x <= 0, Abweichungen außerhalb von min/max: x < -100
        ...
        if ausgabe['slope'] < 0:
            slope = -5000*(max_gens - num_gens)/float(max_gens)
        else:
            if s< slope_max and s > slope_min:  p = 0
            else:                   p = -100    
    ...
    print "Bewertung: \napw = "+str(apw)+"\nslope = "+str(slope)+"\nai = "+str(ai)
    #del m_apw; del apw_max; del apw_min
    #del s; del slope_min; del slope_max
    #del a; del ai_min; del ai_max
    #del max_gens; del num_gens
...
def evaluate_B(args):
    ...
    ### Aufruf von analyze_Nonburst
    ausgabe = analyze_Neurons.analyze_Burst()
    if ausgabe['P'] == 0:
        return {'P':0}      
    max_gens = float(args.get('max_generations'))
    num_gens = float(args['_ec'].num_generations)
    ...
    ### Normierung: Maximum: x = 0, Abweichungen im Rahmen von min/max: -100 <= x <= 0, Abweichungen außerhalb von min/max: x < -100
        ...
        del apw_CH; del ibf_CH; del ir_CH
    print "Bewertungen: \napw = "+str(apw)+"\nibf = "+str(ibf)+"\nir = "+str(ir)        
    #del m_ir; del ir_max; del ir_min; del m_ibf; del ibf_max; del ibf_min
    #del m_apw; del apw_max; del apw_min
    #del max_gens; del num_gens
...
def evaluate_param(candidates, args):
    ...
    if args.get('BS') == 1:
        subprocess.check_call(['/home/kloskowski/Desktop/Python-skript/neuroConstruct_1.6.0/nC.sh', 
                    '-python', 
                    '/home/kloskowski/Desktop/Python-skript/GenAlg/Programm/MultiConductance.py']); sleep(10) #damit er fertig läuft
    else:
    ...
    for i in range(len(candidates)):
        ...
        ISI = analyze_Neurons.analyze_ISI(i)
        hist = ISI['hist']
...
###################################### MUTATE_UNIFORM #####################################################
# nach der inspyred-funktion, die nur nicht funktioniert
def nuMutation(random, candidate, args):
    ...
    for i, (c,lo,hi) in enumerate(zip(candidate, l_bound, u_bound)):
        ...
        mutant[i] = new_value
    #del l_bound; del u_bound; del num_gens; del max_gens; del strength
    #del exponent
    return mutant
#endDEF
...
def main(   population, 
    ...
    #           ar  cal cat k2  ka  kahp    kc  kdr km  naf nap pas
        u_bound = [10**-9,  10**-10,10**-10,10**-9, 10**-7, 10**-8, 10**-7, 10**-4, 10**-6, 10**-3, 10**-9, 10**-10]
        l_bound = [10**-9,  10**-10,10**-10,10**-9, 10**-7, 10**-8, 10**-7, 10**-4, 10**-6, 10**-3, 10**-9, 10**-10]
    elif mode == 2:
    #           ar  cal cat k2  ka  kahp    kc  kdr km  naf nap pas
    ...
    if BetrSys == 1:
        final_pop = EC.evolve(generator =  generate_conductance,
                  evaluator =evaluate_param,
                  statistics_file = open('/home/kloskowski/Desktop/Python-skript/GenAlg/Programm/Speicher/inspyred-statistics-{0}.csv'.format(strftime('%m%d-%H%M')),'w'),
                  individuals_file = open('/home/kloskowski/Desktop/Python-skript/GenAlg/Programm/Speicher/inspyred-individuals-{0}.csv'.format(strftime('%m%d-%H%M')),'w'),
                  filename = '/home/kloskowski/Desktop/Python-skript/GenAlg/Programm/Speicher/inspyred-inspyred-statistics-{0}.csv'.format(strftime('%m%d-%H%M')),
                  errorbars = False,
                  lower_bound = l_bound,
                  upper_bound = u_bound,
                  pop_size = population,
    ...
    if BetrSys == 1:
        d = open("./GenAlg/Programm/Ergebnisse/ErgebnisDens.txt", "a")
    else:
