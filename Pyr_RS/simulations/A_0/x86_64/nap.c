	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 nap /home/kloskowski/Desktop/GenAlg-Kloskowski/Pyr_RS/generatedNEURON/x86_64/nap.mod\n");
...
static int _reset;
static char *modelname = "Channel: nap";
...
static int  _f_rates ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lv ;
 {
  ...
  _ltemp_adj_m = 1.0 ;
   if ( _lv < - 40.0 ) {
     _ltau = 0.025 + 0.14 * ( exp ( ( _lv + 40.0 ) / 10.0 ) ) ;
     }
   else {
     _ltau = 0.02 + 0.145 * ( exp ( - 1.0 * ( _lv + 40.0 ) / 10.0 ) ) ;
     }
