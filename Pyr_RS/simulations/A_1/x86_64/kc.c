	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 5);
 	...
 	ivoc_help("help ?1 kc /home/kloskowski/Desktop/GenAlg-Kloskowski/Pyr_RS/generatedNEURON/x86_64/kc.mod\n");
...
static int _reset;
static char *modelname = "Channel: kc";
...
/*CVODE*/
 ...
 static double _mfac_rates, _tmin_rates;
  ...
  if (_maktable) { double _x, _dx; _maktable=0;
   _tmin_rates =  - 120.0 ;
   _tmax =  140.0 ;
   _dx = (_tmax - _tmin_rates)/1041.; _mfac_rates = 1./_dx;
   for (_i=0, _x=_tmin_rates; _i < 1042; _x += _dx, _i++) {
...
static int  _f_rates ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lv ;
 {
   double _lalpha , _lbeta , _ltau , _linf , _lgamma , _lzeta , _ltemp_adj_m , _lA_alpha_m , _lB_alpha_m , _lVhalf_alpha_m , _lA_beta_m , _lB_beta_m , _lVhalf_beta_m ;
  _ltemp_adj_m = 1.0 ;
   if ( _lv < - 10.0 ) {
     _lalpha = ( 2.0 / 37.95 ) * ( exp ( ( ( _lv + 50.0 ) / 11.0 ) - ( ( _lv + 53.5 ) / 27.0 ) ) ) ;
     }
   else {
     _lalpha = 2.0 * ( exp ( ( ( - 1.0 * _lv ) - 53.5 ) / 27.0 ) ) ;
     }
   if ( _lv < - 10.0 ) {
     _lbeta = 2.0 * ( exp ( ( ( - 1.0 * _lv ) - 53.5 ) / 27.0 ) ) - _lalpha ;
     }
   else {
     _lbeta = 0.0 ;
     }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   double _lg_factor , _lca_conc ;
 gion = gmax * ( pow( ( m ) , 1.0 ) ) ;
   _lca_conc = cai / 1000000.0 ;
   if ( _lca_conc < 0.00025 ) {
     _lg_factor = ( _lca_conc / 0.00025 ) ;
     }
   else {
     _lg_factor = 1.0 ;
     }
...
static _initlists(){
 ...
 _slist1[0] = &(m) - _p;  _dlist1[0] = &(Dm) - _p;
   _t_minf = makevector(1042*sizeof(double));
   _t_mtau = makevector(1042*sizeof(double));
_first = 0;
}
