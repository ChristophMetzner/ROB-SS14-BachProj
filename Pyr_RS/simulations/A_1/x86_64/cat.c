...
#define v _p[12]
#define _g _p[13]
#define _ion_ecat	*_ppvar[0]._pval
#define _ion_icat	*_ppvar[1]._pval
#define _ion_dicatdv	*_ppvar[2]._pval
	...
	Prop *_prop;
{
 ...
 nrn_promote(prop_ion, 0, 1);
 	_ppvar[0]._pval = &prop_ion->param[0]; /* ecat */
 	_ppvar[1]._pval = &prop_ion->param[3]; /* icat */
 	_ppvar[2]._pval = &prop_ion->param[4]; /* _ion_dicatdv */
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 4);
 	...
 	ivoc_help("help ?1 cat /home/kloskowski/Desktop/GenAlg-Kloskowski/Pyr_RS/generatedNEURON/x86_64/cat.mod\n");
...
static int  _f_rates ( _p, _ppvar, _thread, _nt, _lv ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lv ;
 {
  ...
  _ltemp_adj_m = 1.0 ;
   _ltemp_adj_h = 1.0 ;
   _ltau = 0.204 + 0.333 / ( ( exp ( ( 15.8 + _lv ) / 18.2 ) ) + ( exp ( ( - _lv - 131.0 ) / 16.7 ) ) ) ;
   ...
   minf = _linf ;
   if ( _lv < - 81.0 ) {
     _ltau = 0.333 * ( exp ( ( _lv + 466.0 ) / 66.6 ) ) ;
     }
   else {
     _ltau = 9.32 + 0.333 * ( exp ( ( - _lv - 21.0 ) / 10.5 ) ) ;
     }
...
static int _ode_matsol(_NrnThread* _nt, _Memb_list* _ml, int _type) {
 ...
 static void _update_ion_pointer(Datum* _ppvar) {
   nrn_update_ion_pointer(_cat_sym, _ppvar, 0, 0);
   nrn_update_ion_pointer(_cat_sym, _ppvar, 1, 3);
   nrn_update_ion_pointer(_cat_sym, _ppvar, 2, 4);
 }
